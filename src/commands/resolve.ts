...
export abstract class resolve {
  ...
  async summary(interaction: CommandInteraction) {
    const items = await getAllItems(interaction.guildId);
    const expenses = await getAllExpenses(interaction.guildId);
  ...
  async pay(
    ...
    if (!doesUserExist(interaction.guildId, person.id))
    ...
    if (collected.get("❌")) {
      ...
      payBack(interaction.guildId, person.id)
        .then(() => {
